...
public class PatternLockView extends View {
    ...
    private List<PatternLockViewListener> mPatternListeners = new ArrayList<>();
    private ArrayList<Dot> mPattern = new ArrayList<>(mPatternSize);//被点击过点的集合
    private boolean[][] mPatternDrawLookup;//记录那个点被选中
    private String mInitPointString;
    ...
    private PatternLockCheckListener mPatternLockCheckListener;
    ...
    public PatternLockView(Context context, AttributeSet attrs) {
        ...
        try {
            ...
            mPathWidth = (int) typedArray.getDimension(R.styleable.PatternLockView_pathWidth,
                    PatternLockResourceUtils.getDimensionInPx(getContext(), R.dimen.pattern_lock_path_width));
            mNormalDotStateColor = typedArray.getColor(R.styleable.PatternLockView_normalDotStateColor,
                    PatternLockResourceUtils.getColor(getContext(), R.color.col_PatternLockNormalDotStateColor));
            mCorrectDotStateColor = typedArray.getColor(R.styleable.PatternLockView_correctDotStateColor,
                    PatternLockResourceUtils.getColor(getContext(), R.color.col_PatternLockNormalCorrectLineStateColor));
            mCorrectDotStrokeColor = typedArray.getColor(R.styleable.PatternLockView_correctDotStrokeStateColor,
                    PatternLockResourceUtils.getColor(getContext(), R.color.col_PatternLockNormalCorrectStrokeStateColor));
            ...
            mWrongLineStateColor = typedArray.getColor(R.styleable.PatternLockView_wrongLineStateColor,
                    PatternLockResourceUtils.getColor(getContext(), R.color.col_PatternLockNormalWrongStateColor));
            ...
            mWrongDotStrokeStateColor = typedArray.getColor(R.styleable.PatternLockView_wrongDotStrokeStateColor,
                    PatternLockResourceUtils.getColor(getContext(), R.color.col_PatternLockNormalWrongStrokeStateColor));
            mDotNormalSize = (int) typedArray.getDimension(R.styleable.PatternLockView_dotNormalSize,
                    PatternLockResourceUtils.getDimensionInPx(getContext(), R.dimen.pattern_lock_dot_size));
            mDotSelectedSize = (int) typedArray.getDimension(R.styleable.PatternLockView_dotSelectedSize,
                    PatternLockResourceUtils.getDimensionInPx(getContext(), R.dimen.pattern_lock_dot_selected_size));
            mDotAnimationDuration = typedArray.getInt(R.styleable.PatternLockView_dotAnimationDuration,
                    DEFAULT_DOT_ANIMATION_DURATION);

            isCheckMode = typedArray.getBoolean(R.styleable.PatternLockView_checkMode,
                    false);
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
    ...
    private void initView() {
        ...
        if (isCheckMode) {
            addPatternLockListener(mDefaultPatternLockViewListener);
        }
...
//                        mInProgressX, mInProgressY, lastX, lastY) * 255f));
    ...
    public void setErrorPointString(String pointString) {
        setPointString(pointString, PatternViewMode.WRONG);
    }
    ...
    public void setPointString(String mInitPointString, int state) {
        this.mInitPointString = mInitPointString;

        clearPattern();
        mPatternViewMode = PatternViewMode.CORRECT;
        try {
            if (!TextUtils.isEmpty(mInitPointString)) {

                String[] value = mInitPointString.split(",");
                if (value.length > 0) {
                    for (String aValue : value) {
                        addCellToPattern(Dot.of(Integer.parseInt(aValue)));
                    }
                }
                if (!mPattern.isEmpty()) {
                    mPatternInProgress = false;
    ...
    public PatternLockCheckListener getmPatternLockCheckListener() {
        return mPatternLockCheckListener;
    }

    public void setmPatternLockCheckListener(PatternLockCheckListener mPatternLockCheckListener) {
        this.mPatternLockCheckListener = mPatternLockCheckListener;
    }
    ...
    private PatternLockViewListener mDefaultPatternLockViewListener = new PatternLockViewListener() {
        ...
        public void onComplete(List<PatternLockView.Dot> progressPattern) {
            ...
            String password = PatternLockUtils.patternToString(PatternLockView.this, progressPattern);
            if (TextUtils.isEmpty(password)) {
                return;
            }
            password = PatternLockUtils.patternToStringLast(PatternLockView.this, progressPattern);
            String[] value = password.split(",");
            if (value.length >= 4) {
                if (mPatternLockCheckListener != null) {
                    mPatternLockCheckListener.onComplete(password.replace(",", ""));
                }
                mPatternViewMode = PatternViewMode.CORRECT;
            } else {
                mPatternViewMode = PatternViewMode.WRONG;
