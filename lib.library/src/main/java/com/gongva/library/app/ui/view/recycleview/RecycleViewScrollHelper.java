...
/**
 * https://github.com/CrazyTaro/recycle_header_adapter
 *
 * @data 2019/3/28
 */
public class RecycleViewScrollHelper extends RecyclerView.OnScrollListener {
        ...
        if (layoutManager instanceof LinearLayoutManager) {
            ...
            if (newState == RecyclerView.SCROLL_STATE_IDLE) {
                ...
                if (!mIsCheckTopFirstBottomAfter) {
                    //先检测底部
                    if (this.checkIfScrollToBottom(recyclerView, firstItemPosition, lastItemPosition, adapter.getItemCount())) {
                        //若检测滑动到底部时,判断是否需要同时检测滑动到顶部
                        if (mIsCheckTopBottomTogether) {
                            //检测是否滑动到顶部
                            this.checkIfScrollToTop(recyclerView, firstItemPosition, lastItemPosition);
                    ...
                    } else if (this.checkIfScrollToTop(recyclerView, firstItemPosition, lastItemPosition)) {
                        //当未检测滑动到底部时,再检测是否滑动到顶部
                        return;
                    }
                } else {
                    //先检测是否滑动到顶部
                    if (this.checkIfScrollToTop(recyclerView, firstItemPosition, lastItemPosition)) {
                        if (mIsCheckTopBottomTogether) {
                            //检测是否滑动到底部
                            this.checkIfScrollToBottom(recyclerView, firstItemPosition, lastItemPosition, adapter.getItemCount());
                            return;
                        } else {
                            //若不需要同时检测,直接返回
                            return;
                        }
                    } else if (this.checkIfScrollToBottom(recyclerView, firstItemPosition, lastItemPosition, adapter.getItemCount())) {
            ...
            //其它任何情况
            mScrollPositionChangedListener.onScrollToUnknown(false, firstItemPosition, false, lastItemPosition);
        }
    }
     ...
     * @return
     */
    private boolean checkIfScrollToTop(RecyclerView recyclerView, int firstItemPosition, int lastItemPosition) {
        if (firstItemPosition == 0) {
            if (mIsCheckTopFullRecycle) {
                ...
                if (top >= topEdge && bottom > bottomEdge) {
                    mScrollPositionChangedListener.onScrollToTop(firstItemPosition, lastItemPosition);
                    return true;
                } else {
                    mScrollPositionChangedListener.onScrollToUnknown(true, firstItemPosition, false, lastItemPosition);
                }
            } else {
                mScrollPositionChangedListener.onScrollToTop(firstItemPosition, lastItemPosition);
                return true;
     ...
     * @return
     */
    private boolean checkIfScrollToBottom(RecyclerView recyclerView, int firstItemPosition, int lastItemPosition, int itemCount) {
        if (lastItemPosition + 1 == itemCount) {
            ...
            if (mIsCheckBottomFullRecycle) {
                ...
                if (bottom <= bottomEdge && top < topEdge) {
                    mScrollPositionChangedListener.onScrollToBottom(firstItemPosition, lastItemPosition);
                    return true;
                } else {
                    mScrollPositionChangedListener.onScrollToUnknown(false, firstItemPosition, true, lastItemPosition);
                }
            } else {
                mScrollPositionChangedListener.onScrollToBottom(firstItemPosition, lastItemPosition);
                return true;
    ...
    public interface OnScrollPositionChangedListener {
         ...
         * 滑动到顶部的回调事件
         */
        public void onScrollToTop(int firstPosition, int lastPosition);

        /**
         * 滑动到底部的回调事件
         */
        public void onScrollToBottom(int firstPosition, int lastPosition);
         ...
         */
        public void onScrollToUnknown(boolean isTopViewVisible, int firstPosition, boolean isBottomViewVisible, int lastPosition);
    }

    public static class OnScrollPositionChangedAdapter implements OnScrollPositionChangedListener {
